...
//                 uid: profile.id,
//             })
//         await db.collection('following').doc(profile.id).collection('userFollowers')
//             .doc(profile.id).set({
//                 displayName: user.displayName,
...
//     const user = firebase.auth().currentUser;
//     try {
//         await db.collection('following').doc(user.uid).collection('userFollowing')
...
export function getFollowingDoc(profileId) {
    const userUid = firebase.auth().currentUser.uid;
    return db.collection('following').doc(userUid)
        .collection('userFollowing').doc(profileId).get();
}